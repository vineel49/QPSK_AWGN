...
int sim_runs=pow(10,3); /*simulation runs*/
int frame_size=1024; /*frame size */
noise_var_1D = (double)0.5*2/(2*pow(10,0.1*SNR_dB));
/*------------------------------------------------------*/
int C_Ber=0; /*total number of channel errors*/
...
re_awgn[i3] = sqrt(noise_var_1D)*x*f;
im_awgn[i3] = sqrt(noise_var_1D)*y*f;
}
...
err1 = a_hat[2*i5]^a[2*i5];
err2 = a_hat[2*i5+1]^a[2*i5+1];
